...
class Token {
    ...
    constructor({ token_id, volunteer_id, token }){
        this.token_id = token_id;
        this.volunteer_id = volunteer_id;
        this.token = token;
    }

    static async create(volunteer_id) {
        const token = uuidv4();
        const response = await db.query("INSERT INTO token (volunteer_id, token) VALUES ($1, $2) RETURNING token_id;",
            [volunteer_id, token]);
